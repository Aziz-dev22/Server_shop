...
# ---------------------------------------------------------------------------
async def nowpayments_create_payment(api_key: str, amount_usd: float, order_id: str):
    async with httpx.AsyncClient(timeout=20) as client:
        r = await client.post(
...
async def wallet_charge_nowpayments(call: CallbackQuery, state: FSMContext):
    ...
    order_id = f"w{call.from_user.id}{int(datetime.utcnow().timestamp())}"
    try:
        payment_id, pay_address, pay_amount, pay_currency = await nowpayments_create_payment(nowpayments_key, amount, order_id)
    except Exception as e:
        await call.message.answer(f"❌ خطا در ساخت فاکتور پرداخت: {e}")
        await call.answer()
        return
...
async def adm_nowpayments_menu(call: CallbackQuery):
    ...
    await call.message.edit_text(
        ...
        f"کاربران مبلغ را به آدرس تولیدشده به ارز {NOWPAYMENTS_PAY_CURRENCY.upper()} واریز می‌کنند و ربات "
        "به‌صورت خودکار وضعیت را چک و کیف پول را شارژ می‌کند.\n"
