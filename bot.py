...
async def oxapay_poller(bot: Bot):
    """هر ۴۵ ثانیه وضعیت فاکتورهای ارزی در انتظار را چک می‌کند (بدون نیاز به وب‌هوک/دامنه)."""
    while True:
        await asyncio.sleep(45)
        try:
            async with aiosqlite.connect(DB_PATH) as db:
                api_key = await get_setting(db, "oxapay_api_key")
                if not api_key:
                    continue
                cur = await db.execute(
                    "SELECT id, user_id, amount, track_id FROM wallet_requests "
                    "WHERE status='pending' AND method='auto' AND track_id IS NOT NULL"
                )
                rows = await cur.fetchall()
            for req_id, user_id, amount, track_id in rows:
                try:
                    status = await oxapay_check_status(api_key, track_id)
                except Exception:
                    continue
                if "paid" in status.lower() or "complet" in status.lower() or "confirm" in status.lower():
                    async with aiosqlite.connect(DB_PATH) as db:
            ...
            logger.warning(f"oxapay poller error: {e}")
...
async def wallet_charge_amount(message: Message, state: FSMContext):
    ...
    if oxapay_key:
        kb = InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(text="💠 پرداخت خودکار با ارز دیجیتال", callback_data="chg_auto")],
                [InlineKeyboardButton(text="🏦 واریز دستی (کارت/ولت)", callback_data="chg_manual")],
            ]
        )
        await message.answer("روش شارژ را انتخاب کنید:", reply_markup=kb)
    else:
        await charge_manual_prompt(message, state)


@router.callback_query(F.data == "chg_auto")
async def wallet_charge_auto(call: CallbackQuery, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            ...
            (call.from_user.id, amount, "auto", track_id, datetime.utcnow().isoformat()),
        )
        await db.commit()
    await state.clear()
...
async def buy_location(call: CallbackQuery, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        token = await get_active_token(db)
        cur = await db.execute("SELECT id, title, cpu, ram, disk, server_type, markup_percent FROM plans WHERE active=1")
...
# ---------------------------------------------------------------------------
# پنل ادمین: همه سرورها (دسترسی کامل ادمین)
# ---------------------------------------------------------------------------
...
async def adm_servers(call: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
            "SELECT id, name, ip, user_id, location, status FROM servers ORDER BY id DESC LIMIT 30"
        )
        servers = await cur.fetchall()
    if not servers:
        ...
        await call.answer()
        return
    await call.message.edit_text(f"🖥 {len(servers)} سرور آخر (با دکمه‌های مدیریت کامل):", reply_markup=back_inline())
    for s in servers:
        text = f"🖥 {s[1]}\nآی‌پی: {s[2]}\nمالک: {s[3]}\nدیتاسنتر: {s[4]}\nوضعیت: {s[5]}"
...
async def adm_plans(call: CallbackQuery):
    if not is_admin(call.from_user.id):
        return
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT id, title, server_type, markup_percent, active FROM plans ORDER BY id")
        plans = await cur.fetchall()
    lines = ["🧩 پلن‌های فعلی (قیمت به‌صورت زنده از Hetzner + درصد سود محاسبه می‌شود):\n"]
    for p in plans:
        lines.append(f"• #{p[0]} {p[1]} ({p[2]}) | سود: %{p[3]:g} | {'✅' if p[4] else '❌'}")
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
...
async def adm_plan_markup(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "INSERT INTO plans (server_type, title, cpu, ram, disk, markup_percent, active) VALUES (?,?,?,?,?,?,1)",
            (data["server_type"], data["title"], data["cpu"], data["ram"], data["disk"], markup),
        )
        await db.commit()
    await state.clear()
    await message.answer("✅ پلن جدید اضافه شد. قیمت نهایی همیشه زنده از Hetzner محاسبه می‌شود.", reply_markup=main_menu_kb(True))
