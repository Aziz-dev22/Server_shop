...
def page(title: str, body: str, path: str = "") -> HTMLResponse:
    nav_html = "".join(
        f'<a class="nav-link{" active" if path == href else ""}" href="{href}">{label}</a>'
...
<style>
  * {{ box-sizing: border-box; }}
  body {{ font-family: Tahoma, sans-serif; background:#0f1420; color:#e6e9ef; margin:0; }}
  .wrap {{ display:flex; min-height:100vh; }}
  .sidebar {{ width:220px; background:#161c2c; padding:18px 12px; flex-shrink:0; }}
  .sidebar h2 {{ font-size:15px; color:#7ea6ff; margin:4px 6px 18px; }}
  .nav-link {{ display:block; padding:10px 12px; color:#c6cbdb; text-decoration:none; border-radius:8px; margin-bottom:4px; font-size:14px; }}
  .nav-link:hover {{ background:#212843; }}
  .nav-link.active {{ background:#2c5cff; color:#fff; }}
  .logout {{ display:block; margin-top:20px; padding:10px 12px; color:#ff8080; text-decoration:none; font-size:13px; }}
  .content {{ flex:1; padding:26px 30px; max-width:1100px; }}
  h1 {{ font-size:20px; margin-top:0; color:#fff; }}
  .card {{ background:#161c2c; border-radius:12px; padding:18px; margin-bottom:18px; border:1px solid #232b45; }}
  table {{ width:100%; border-collapse: collapse; font-size:13px; }}
  th, td {{ text-align:right; padding:8px 6px; border-bottom:1px solid #232b45; }}
  th {{ color:#8b93ac; font-weight:normal; }}
  .btn {{ display:inline-block; background:#2c5cff; color:#fff; border:none; padding:7px 14px; border-radius:7px; font-size:13px; cursor:pointer; text-decoration:none; }}
  .btn.small {{ padding:4px 9px; font-size:12px; margin:2px; }}
  .btn.red {{ background:#e5484d; }}
  .btn.green {{ background:#2fbf71; }}
  .btn.gray {{ background:#3a4160; }}
  input, select, textarea {{ background:#0f1420; border:1px solid #2c3350; color:#e6e9ef; padding:8px 10px; border-radius:7px; font-size:13px; width:100%; margin-bottom:10px; font-family:inherit; }}
  label {{ font-size:13px; color:#9aa2bd; display:block; margin-bottom:4px; }}
  .stat-grid {{ display:flex; gap:14px; flex-wrap:wrap; }}
  .stat-box {{ background:#161c2c; border:1px solid #232b45; border-radius:12px; padding:16px 22px; min-width:140px; }}
  .stat-box .num {{ font-size:22px; color:#7ea6ff; font-weight:bold; }}
  .stat-box .lbl {{ font-size:12px; color:#9aa2bd; margin-top:4px; }}
  .badge {{ padding:2px 8px; border-radius:20px; font-size:11px; }}
  .badge.ok {{ background:#123; color:#2fbf71; }}
  .badge.off {{ background:#331; color:#ff8080; }}
  .msg {{ padding:10px 14px; border-radius:8px; margin-bottom:14px; font-size:13px; }}
  .msg.ok {{ background:#123822; color:#2fbf71; }}
  .msg.err {{ background:#3a1620; color:#ff8080; }}
  form.inline {{ display:inline; }}
  .login-wrap {{ display:flex; align-items:center; justify-content:center; height:100vh; }}
  .login-box {{ background:#161c2c; padding:34px; border-radius:14px; width:320px; border:1px solid #232b45; }}
</style>
</head>
<body>
<div class="wrap">
...
async def login_form(request: Request):
    ...
    <h2 style="text-align:center;margin-bottom:20px;">ورود به پنل مدیریت</h2>
    {msg}
    <form method="post" action="/login">
      ...
      <button class="btn" style="width:100%;margin-top:6px;" type="submit">ورود</button>
    </form>
    </div></div>
    """
    return HTMLResponse(f"<html lang='fa' dir='rtl'><head><meta charset='utf-8'><title>ورود</title><style>body{{font-family:Tahoma,sans-serif;background:#0f1420;color:#e6e9ef;margin:0;}} .btn{{background:#2c5cff;color:#fff;border:none;padding:9px;border-radius:7px;cursor:pointer;}} input{{background:#0f1420;border:1px solid #2c3350;color:#e6e9ef;padding:9px 10px;border-radius:7px;width:100%;margin-bottom:12px;box-sizing:border-box;}} .msg.err{{background:#3a1620;color:#ff8080;padding:10px;border-radius:8px;margin-bottom:14px;}}</style></head><body>{body}</body></html>")
...
async def plans_list(request: Request):
    ...
    <div class="card"><table>
      ...
      {rows or "<tr><td colspan=7>پلنی تعریف نشده.</td></tr>"}
    </table></div>
    """
...
async def settings_page(request: Request):
    ...
    <div class="card">
      ...
      <p style="color:#9aa2bd;font-size:13px;">وضعیت فعلی: {"فعال (..." + esc(oxa_key[-4:]) + ")" if oxa_key else "غیرفعال"}</p>
